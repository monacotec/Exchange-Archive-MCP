...
- **[HIGH · confirmed] App-only credential + caller-chosen mailbox = confused deputy** — [`gi-foundry/exchange-mcp/function_app.py:67`](gi-foundry/exchange-mcp/function_app.py) builds a `ClientSecretCredential` with tenant-wide `Mail.Read`, then every tool reads whatever `user_email` the caller passed (lines 136, 216, 283). The server uses *its own* high-privilege identity to satisfy a mailbox the *caller* named.
...
- **[LOW · plausible] `MCP_ALLOWED_MAILBOXES` fails open** — [`function_app.py:93`](gi-foundry/exchange-mcp/function_app.py) returns "allow" when the env var is unset. It's a per-deployment global list, never tied to the caller.
...
- **[HIGH · confirmed] `x-functions-key` is the only transport auth** — [`function_app.py:83`](gi-foundry/exchange-mcp/function_app.py) uses `AuthLevel.FUNCTION`: one static shared key, no OAuth/JWT, no `aud`/`iss`/`exp`, no per-client identity, no rotation, no RBAC. The key lives in Claude Desktop config, `.vscode/mcp.json`, API Center, and CI — many copies, one blast radius.
- **[HIGH · confirmed] That endpoint is public** — [`infra/modules/functionapp.bicep:73`](gi-foundry/exchange-mcp/infra/modules/functionapp.bicep) sets `publicNetworkAccess: 'Enabled'` with no `ipSecurityRestrictions`, no VNet integration, no Private Endpoint.
...
- **[LOW · plausible] Rotation keeps the previous secret valid** — [`Rotate-MCPClientSecret.ps1:69`](gi-foundry/scripts/Rotate-MCPClientSecret.ps1) prunes by count (keep newest 2), so the prior secret stays live. Acceptable for zero-downtime overlap, but shorten the overlap window and give the retiring secret a near-term `EndDateTime` instead of the default +1 year.
- **[LOW · plausible] Runtime storage account is publicly reachable** — [`functionapp.bicep:39`](gi-foundry/exchange-mcp/infra/modules/functionapp.bicep) sets no `networkAcls`/Private Endpoint.
...
- **[HIGH · confirmed] `listKeys()` written into a Key Vault secret's `value`** — [`foundry-iac/modules/openai.bicep:89`](gi-foundry/foundry-iac/modules/openai.bicep). ARM captures fully-resolved property values in the deployment operation record, retrievable by anyone with resource-group Reader.
  **Gotcha:** the *identical* anti-pattern is in [`foundry.bicep:43`](gi-foundry/foundry-iac/modules/foundry.bicep) (`credentials.key = listKeys(...)`). Fixing only `openai.bicep` leaves the leak. Fix *both*.
- **[HIGH · confirmed] `what-if --result-format FullResourcePayloads` prints resolved secrets to CI logs** — [`bicep-validate.yml:35`](gi-foundry/foundry-iac/.github/workflows/bicep-validate.yml). Once the OpenAI resource exists, `listKeys()` resolves and the key renders into the retained GitHub Actions log.
...
- **[HIGH · confirmed] Storage account key embedded as a plaintext connection string** — [`functionapp.bicep:80`](gi-foundry/exchange-mcp/infra/modules/functionapp.bicep). `AzureWebJobsStorage` inlines `storage.listKeys().keys[0].value`; `allowSharedKeyAccess` is left default-`true`. That key can read/overwrite the *host keys* (including the `mcp_extension` key), i.e. take over the function.
...
- **[MEDIUM · confirmed] CI exposes `AZURE_CREDENTIALS` to PR-triggered runs** — [`bicep-validate.yml:11`](gi-foundry/foundry-iac/.github/workflows/bicep-validate.yml).
...
- **[MEDIUM · confirmed] Live OpenAI key written to a plaintext temp file** — [`Rotate-OAIKey.ps1:79`](gi-foundry/scripts/Rotate-OAIKey.ps1) serializes the key into `%TEMP%\*.json` for `az ml connection update`.
...
- **[MEDIUM · plausible] OpenAI key stored inline as `authType: 'ApiKey'`** — [`foundry.bicep:43`](gi-foundry/foundry-iac/modules/foundry.bicep). A static, non-rotating, full-data-plane credential.
...
- **[LOW · plausible] `isSharedToAll: true` widens the key's blast radius** — [`foundry.bicep:41`](gi-foundry/foundry-iac/modules/foundry.bicep). Moot once you go keyless (nothing to share). If you must keep a key, pair `isSharedToAll: false` with explicit per-project grants.
...
- **[MEDIUM · confirmed] Unbounded `ReadToEnd` on the request body** — [`04-ToolPermissions.ps1:214`](mcp-security-ps1/04-ToolPermissions.ps1). A multi-GB body is loaded into a string, then copied again by `ConvertFrom-Json`.
  **Gotcha — the naive fix truncates valid input:** a single `Stream.Read(buffer,0,max)` is *not* guaranteed to fill the buffer, and `ContentLength64` is `-1` for chunked transfers (bypassing a `-gt max` guard). Use a **bounded read loop** that aborts once total bytes exceed the ceiling, plus a read timeout:
  ...
  ```
- **[MEDIUM · plausible] No rate limiting / circuit breaker on tools** — [`function_app.py:112`](gi-foundry/exchange-mcp/function_app.py).
...
- **[MEDIUM · confirmed] Raw `str(exc)` returned to the caller** — [`function_app.py:157`](gi-foundry/exchange-mcp/function_app.py) (also 244, 328). Graph/Kiota exceptions embed UPNs, mailbox GUIDs, request URLs, tenant IDs. Return `{"error": True, "message": "Request failed", "correlation_id": <uuid>}`; keep the full trace in `logger.exception`.
- **[MEDIUM · confirmed] No per-call audit fields** — [`function_app.py:156`](gi-foundry/exchange-mcp/function_app.py). §11 requires `timestamp, tool_name, user_identity, input_summary, output_summary` per call.
...
- **[MEDIUM · confirmed] `$search` value only wrapped in quotes** — [`function_app.py:132`](gi-foundry/exchange-mcp/function_app.py) builds `f'"{query}"'`. A `"` in `query` closes the phrase and injected KQL (`from:`, `subject:`, `AND/OR/NOT`) is interpreted.
...
- **[LOW · plausible] Graph/Exchange sessions not disconnected on failure** — [`Register-EntraApp.ps1:34`](gi-foundry/scripts/Register-EntraApp.ps1), [`Rotate-MCPClientSecret.ps1:38`](gi-foundry/scripts/Rotate-MCPClientSecret.ps1), [`Set-ApplicationAccessPolicy.ps1:47`](gi-foundry/scripts/Set-ApplicationAccessPolicy.ps1). Under `$ErrorActionPreference='Stop'`, an exception skips the trailing `Disconnect-*`. Wrap the body in `try/finally` with a guaranteed disconnect (per your global CLAUDE.md rule).
- **[LOW · confirmed] `GetTempFileName()` orphans a zero-byte file** — [`Rotate-OAIKey.ps1:77`](gi-foundry/scripts/Rotate-OAIKey.ps1). `GetTempFileName()` *creates* a `.tmp`, then `+ '.json'` points elsewhere, leaking one file per run. Build the path with `[guid]::NewGuid()` instead and clean up in `finally`.
...
| 2 | High | C | foundry-python | App-only credential + caller-chosen mailbox (confused deputy) | [function_app.py:67](gi-foundry/exchange-mcp/function_app.py) |
| 3 | High | C | foundry-python | Shared `x-functions-key` is the only transport auth | [function_app.py:83](gi-foundry/exchange-mcp/function_app.py) |
| 4 | High | C | foundry-infra | MCP endpoint public, static key only | [functionapp.bicep:73](gi-foundry/exchange-mcp/infra/modules/functionapp.bicep) |
| 5 | High | C | foundry-infra | Storage key inline plaintext connection string | [functionapp.bicep:80](gi-foundry/exchange-mcp/infra/modules/functionapp.bicep) |
| 6 | High | C | foundry-iac-core | `listKeys()` into KV secret `value` → deployment history | [openai.bicep:89](gi-foundry/foundry-iac/modules/openai.bicep) |
| 7 | High | C | foundry-iac-core | `what-if FullResourcePayloads` prints key to CI log | [bicep-validate.yml:35](gi-foundry/foundry-iac/.github/workflows/bicep-validate.yml) |
...
| 12 | Med | C | foundry-iac-core | CI exposes `AZURE_CREDENTIALS` to PR runs | [bicep-validate.yml:11](gi-foundry/foundry-iac/.github/workflows/bicep-validate.yml) |
| 13 | Med | P | foundry-iac-core | OpenAI key inline `authType: ApiKey` | [foundry.bicep:43](gi-foundry/foundry-iac/modules/foundry.bicep) |
| 14 | Med | P | foundry-iac-core | Key Vault has no diagnostic logging | [keyvault.bicep:14](gi-foundry/foundry-iac/modules/keyvault.bicep) |
| 15 | Med | C | foundry-python | `$search` value not escaped (KQL injection) | [function_app.py:132](gi-foundry/exchange-mcp/function_app.py) |
| 16 | Med | C | foundry-python | Raw `str(exc)` returned to caller | [function_app.py:157](gi-foundry/exchange-mcp/function_app.py) |
| 17 | Med | C | foundry-python | No user identity in any log line | [function_app.py:156](gi-foundry/exchange-mcp/function_app.py) |
| 18 | Med | P | foundry-python | No rate limit / Graph circuit breaker | [function_app.py:112](gi-foundry/exchange-mcp/function_app.py) |
...
| 22 | Med | C | ps-scripts | Plaintext OpenAI key to temp file | [Rotate-OAIKey.ps1:79](gi-foundry/scripts/Rotate-OAIKey.ps1) |
...
| 30 | Low | P | foundry-iac-core | `isSharedToAll: true` widens key blast radius | [foundry.bicep:41](gi-foundry/foundry-iac/modules/foundry.bicep) |
| 31 | Low | P | foundry-infra | Runtime storage publicly reachable | [functionapp.bicep:39](gi-foundry/exchange-mcp/infra/modules/functionapp.bicep) |
| 32 | Low | P | foundry-python | `MCP_ALLOWED_MAILBOXES` fails open | [function_app.py:93](gi-foundry/exchange-mcp/function_app.py) |
...
| 43 | Low | C | ps-scripts | `GetTempFileName()` orphans a temp file | [Rotate-OAIKey.ps1:77](gi-foundry/scripts/Rotate-OAIKey.ps1) |
| 44 | Low | P | ps-scripts | Exchange session not disconnected on failure | [Set-ApplicationAccessPolicy.ps1:47](gi-foundry/scripts/Set-ApplicationAccessPolicy.ps1) |
| 45 | Low | P | ps-scripts | Graph session not disconnected (Register) | [Register-EntraApp.ps1:34](gi-foundry/scripts/Register-EntraApp.ps1) |
| 46 | Low | P | ps-scripts | Graph session not disconnected (Rotate secret) | [Rotate-MCPClientSecret.ps1:38](gi-foundry/scripts/Rotate-MCPClientSecret.ps1) |
| 47 | Low | P | ps-scripts | Old client secret stays valid after rotation | [Rotate-MCPClientSecret.ps1:69](gi-foundry/scripts/Rotate-MCPClientSecret.ps1) |
