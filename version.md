...
| `infra/main.bicep` | 3.3.0 | Project tag declared in the shared tags var (ARM replaces tag sets on deploy). (3.2.0: wires Log Analytics workspace ID into keyvault module) |
...
| `infra/modules/sqlhost.bicep` | 1.1.0 | Shared Azure SQL logical server for MCP data stores: Entra-only auth, always-on tier (no serverless auto-pause), databases as an array (one per MCP), firewall allowlist. Deployed independently of azd |
...
| `scripts/Set-ResourceTags.ps1` | 1.0.0 | Tags this project's resources `Project=Exchange Archive MCP` using tag MERGE (never replace); name-pattern selection so a shared RG is not mislabelled; dry-run by default |
