...
| `scripts/Import-ArchiveSearchToSql.ps1` | 1.1.0 | eDiscovery -> SQL loader/verifier: auto-bisects date windows past the 500-item export ceiling (no 100/call cap — reads the report CSV), UTF-8-BOM staging load, MERGE dedupe by internet message id, `-VerifyOnly`. 1.1.0: `-CreateDatabase`, LocalDB pre-start + startup-transient retry, MERGE-proc presence check |
