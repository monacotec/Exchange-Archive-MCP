...
| `scripts/Initialize-McpSqlHost.ps1` | 1.2.0 | Provision/verify the shared SQL host: deploy bicep, verify server+DBs via ARM, grant contained users (Function App MI + access group) by SID, prove connectivity. `-AddDatabase` for the next MCP; `-VerifyOnly` |
